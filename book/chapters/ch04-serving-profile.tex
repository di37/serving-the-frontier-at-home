...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.80\textwidth]{figures/toon-ch04-flag42.png}
\caption{Exactly correct, and exactly wrong. 42 is the largest uniform decode step at the default profile --- six sequences times $k{+}1$ tokens --- so it looks like the right answer and is arrived at honestly. The engine captures on a stride of 8, and an unrounded value falls between the notches: the busiest step runs fully eager, silently, at a measured cost of 12\,\% of six-stream throughput. Rounding up to 48 costs nothing and captures everything.}
\label{fig:toon-flag42}
\end{figure}
